...
def train(loader, model, loss_model, opt, sche, epoch,dep_graph,last_preds,logger,selected_view):
    ...
    for i, (data, label, inc_V_ind, inc_L_ind) in enumerate(loader):
        ...
        z_sample, uniview_mu_list, uniview_sca_list, fusion_z_mu, fusion_z_sca, xr_list, label_emb_sample, pred, xr_p_list, cos_loss, mapped_loss, loss_manifold_p_avg , fusion_p, mapped_fea = model(data_selected,mask=inc_V_ind )
...
def train_step2(loader, model, model_last, loss_model, opt, sche, epoch,dep_graph,last_preds,logger,selected_view, selected_view_last):
    ...
    for i, (data, label, inc_V_ind, inc_L_ind) in enumerate(loader):
        ...
        with torch.no_grad():
            ...
            # 使用第一阶段模型生成当前批次对应的输出
            _, _, _, _, _, _, _, _, xr_p_list, _, mapped_loss, loss_manifold_p_avg , fusion_p, mapped_fea = model_last(data_view_1, mask=inc_V_ind)
        ...
        if epoch<args.pre_epochs:
            ...
            for v in range(len(data_selected)):
                ...
                loss_mse_p += loss_model.weighted_wmse_loss(data_selected[v],xr_p_list[v],inc_V_ind[:,v],reduction='mean')
            assert torch.sum(torch.isnan(loss_mse)).item() == 0
            loss = loss_CL + loss_mse *args.alpha + z_c_loss*args.beta + cohr_loss *args.sigma + loss_mse_p*0.1 + cos_loss*0.1 
        # loss = loss_CL
        opt.zero_grad()
        loss.backward()
        if isinstance(sche,CosineAnnealingWarmRestarts):
...
def test1(loader, model, loss_model, epoch,logger,selected_view):
    ...
    for i, (data, label, inc_V_ind, inc_L_ind) in enumerate(loader):
        ...
        z_sample, uniview_mu_list, uniview_sca_list, fusion_z_mu, fusion_z_sca, xr_list, label_emb_sample, qc_z, xr_p_list, cos_loss, _, _, fusion_p, mapped_fea = model(data_selected,mask=inc_V_ind.to('cuda:0'))
...
def test2(loader, model, model_last, loss_model, epoch,logger,selected_view, selected_view_last):
    ...
    for i, (data, label, inc_V_ind, inc_L_ind) in enumerate(loader):
        ...
        with torch.no_grad():
            ...
            data_view_1 = [data[i] for i in selected_view_last]  # 使用第一阶段的视图选择
            # 使用第一阶段模型生成当前批次对应的输出
            z_sample, uniview_mu_list, uniview_sca_list, fusion_z_mu, fusion_z_sca, xr_list, label_emb_sample, pred, xr_p_list, cos_loss, mapped_loss, loss_manifold_p_avg , fusion_p, mapped_fea = model_last(data_view_1, mask=inc_V_ind)
        ...
        for label_missing_rate in args.label_missing_rates:
            for feature_missing_rate in args.feature_missing_rates:
                ...
                for fold_idx in range(folds_num):
                    ...
                    model_1 = None
                    start = time.time()
                    selected_view1 = [2,3,4,5,1]
                    selected_view2 = [0,2,3,4,5]
