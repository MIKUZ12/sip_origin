...
def train(loader, model, loss_model, opt, sche, epoch,dep_graph,last_preds,logger,selected_view):
    ...
    for i, (data, label, inc_V_ind, inc_L_ind) in enumerate(loader):
        ...
        z_sample, uniview_mu_list, uniview_sca_list, fusion_z_mu, fusion_z_sca, xr_list, label_emb_sample, pred, label_emb,label_emb_var,p_xr_list = model(data_selected,mask=inc_V_ind )
...
def test(loader, model, loss_model, epoch,logger,selected_view):
    ...
    for i, (data, label, inc_V_ind, inc_L_ind) in enumerate(loader):
        ...
        z_sample, uniview_mu_list, uniview_sca_list, fusion_z_mu, fusion_z_sca, xr_list, label_embedding_sample, qc_z, label_emb,label_emb_var, xr_list_views = model(data_selected,mask=inc_V_ind.to('cuda:0'))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--datasets', type=list, default=['pascal07_six_view'])#'mirflickr_six_view','pascal07_six_view' 30
    ...
    parser.add_argument('--epochs', type=int, default=30) #200 for corel5k  100 for iaprtc12 50 for pascal07 30 for espgame
